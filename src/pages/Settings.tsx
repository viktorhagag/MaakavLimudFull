...
export default function SettingsPage() {
  ...
  async function handleLogin() {
    ...
    else alert('נשלח מייל התחברות. בדוק את התיבה ואשר.');
  }
  ...
  function handleExport() {
    ...
    a.href = url; a.download = 'torah-tracker-export.json'; a.click();
  ...
  function handleReset() {
    const ok = confirm('לאפס את כל הנתונים המקומיים? פעולה זו אינה הפיכה.');
    if (ok) { resetAll(); alert('בוצע.'); }
  }

  function toggleCloudSync(next: boolean) {
    setCloudSync(next);
    const s = getSettings();
    setSettings({ ...s, cloudSync: next });
  }
